...
module Lppl
  ...
  # Newey-West (Bartlett-kernel) SE of the mean -- moved VERBATIM to
  # BTC::Stats (M13-1); this delegation keeps every LPPL call site and
  # published number byte-identical. Semantics documented at the new home.
  def newey_west_se(xs, lag:)
    BTC::Stats.newey_west_se(xs, lag: lag)
  end
  ...
  # Downhill-simplex (Nelder-Mead) minimizer -- moved VERBATIM to
  # BTC::Stats (M13-1); delegation keeps LPPL call sites byte-identical.
  # Semantics documented at the new home.
  def nelder_mead(x0, steps, max_iter: 400, tol: 1e-10, &fn)
    BTC::Stats.nelder_mead(x0, steps, max_iter: max_iter, tol: tol, &fn)
  end
