...
class TestDistContract < Minitest::Test
  ...
  HORIZON_KEYS = %w[d forward median median_rwm p05 p25 p75 p95
                    sigma_atm sigma_rw vrp tilt_scale extrapolated
                    degraded].freeze
  EDGE_KEYS = %w[name ts headline date spot edge].freeze
  ...
  def test_dist_edge_contract
    j = run_json('scripts/dist/dist.rb', '--json', '--edge')
    assert_contract_keys EDGE_KEYS, j, 'dist.rb --edge'
    assert_equal 'dist_edge', j['name']
  end
