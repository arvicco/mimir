...
class TestBtcFixtures < Minitest::Test
  ...
  CANNED = {
    ...
    'exchange/balance/list'            => JSON.generate('code' => '0',
      'data' => (1..6).map { |i| { 'exchange_name' => "Ex#{i}", 'total_balance' => 100_000.0 * i,
                                   'balance_change_1d' => -10.0, 'balance_change_7d' => -50.0,
                                   'balance_change_30d' => -200.0, 'balance_change_percent_1d' => -0.01,
                                   'balance_change_percent_7d' => -0.05, 'balance_change_percent_30d' => -0.2 } }),
    'exchange/balance/chart'           => JSON.generate('code' => '0',
      'data' => { 'time_list' => (1..140).map { |i| 1_700_000_000_000 + i * 86_400_000 },
                  'price_list' => (1..140).map { |i| 50_000.0 + i },
                  'data_map' => { 'Ex1' => (1..140).map { |i| 1_000_000.0 - i * 100 },
                                  'Ex2' => (1..140).map { |i| 500_000.0 - i * 50 } } }),
