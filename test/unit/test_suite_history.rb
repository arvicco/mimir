...
class TestSuiteHistory < Minitest::Test
  ...
  def test_both_succeed_ok_and_summary_lines
    ...
    assert_equal 'suite-history OK: 3/3 suites updated (lppl, scenario, positioning)', r[:lines].last
    # runs lppl FIRST (price cache), then scenario, then positioning -- in order.
    assert_equal [LPPL, SCN, POS], calls.map { |c| c.find { |a| a.end_with?('.rb') } }
  end
  ...
  def test_failure_aborts_with_stderr_tail
    ...
    assert_match(/suite-history FAILED: 2\/3 updated, failed: scenario/, r[:lines].last)
  ...
  def test_positioning_failure_is_isolated
    ...
    assert_match(/suite-history FAILED: 2\/3 updated, failed: positioning/, r[:lines].last)
    # all three still ran (sequential isolation).
    assert_equal [LPPL, SCN, POS], calls.map { |c| c.find { |a| a.end_with?('.rb') } }
  end
end
