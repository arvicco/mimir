  ...
  module Health
    ...
    SOURCES = [
      ...
      { name: 'coinmetrics onchain', src: 'scripts/scenario/onchain_value.rb',
        marker: 'CapMrktCurUSD',
        url: -> { CM + '?assets=btc&metrics=CapMrktCurUSD,CapRealUSD,SplyCur,PriceUSD&frequency=1d&page_size=5&start_time=' + (Time.now.utc - 10 * 86_400).strftime('%Y-%m-%d') },
        check: ->(b) { d = JSON.parse(b).fetch('data'); !d.empty? && d.last['CapRealUSD'].to_f > 0 } },
